...
public class pontok2 {
	...
	static void program() {
		...
		do {
			System.out.println("Az első vagy a második input mód? (1-2)");
		...
		System.out.println();
		System.out.println("Szeretne új műveletet megadni?");
		input.nextLine();
		while(true) {
	...
	static ret operandsFirstType() {
		...
		while (true) {
			System.out.println("Milyen műveletet szeretne végrehajtani? (+, -, *, /)");
	...
	static ret operandsSecondType() {
		...
		System.out.println("Adj meg egy műveletet (pl. 1 + 2)");
		Scanner input = new Scanner(System.in);
